...
static u32 buf_2_elliptic_curve_point(  u8* buf,   u32 len, 
	...
	if(pk_algorithm == ECDSA_NISTP224_WITH_SHA224)
		field_size = 28;
	else if(pk_algorithm == ECDSA_NISTP256_WITH_SHA256)
		field_size = 32;
...
static u32 buf_2_ecdsa_signature(  u8* buf,  u32 len,ecdsa_signature* ecdsa_signature, 
	...
	if(NULL == ecdsa_signature->s.buf)
		return -1;
	fill_buf8(ecdsa_signature->s.buf, mbuf, ecdsa_signature->s.len);//填充buf
...
static u32 buf_2_public_key( u8* buf,  u32 len,public_key* public_key){
	...
	switch(public_key->algorithm){
		case ECDSA_NISTP224_WITH_SHA224:
        case ECDSA_NISTP256_WITH_SHA256:
			elliptic_length=buf_2_elliptic_curve_point(mbuf,len,&public_key->u.public_key,public_key->algorithm);
			if(0==elliptic_length)
				return -1;
			bitnum=head_bit_num(mbuf);
			mbuf+=bitnum;
...
//			geographic_region->u.rectangular_region.buf=(rectangular_region*)malloc(sizeof(u8)*geographic_region->u.rectangular_region.len);
			geographic_region->u.rectangular_region.buf=(rectangular_region*)malloc(data_length);
...
//			buf_2_rectangular_region(mbuf,geographic_region->u.rectangular_region.len,(rectangular_region*)&geographic_region->u.rectangular_region);
			for(decode_len_sum=0,i=0;decode_len_sum < data_length;i++){
				decode_len = buf_2_rectangular_region(mbuf,geographic_region->u.rectangular_region.len,
...
static u32 buf_2_psid_priority(u8* buf,u32 len,psid_priority* psid_priority){
	...
	psid_priority->psid= get32(mbuf);
	be_to_host32(psid_priority->psid);
	mbuf+=4;
	size-=4;
...
//	   size-=psid_priority_array->u.permissions_list.len*sizeof(u8);
			...
			psid_priority_array->u.permissions_list.buf=(psid_priority*)malloc(data_length);
			if(NULL == psid_priority_array->u.permissions_list.buf)
				return -1;

			for(decode_len_sum=0,i=0;decode_len_sum < data_length;i++){
				decode_len = buf_2_psid_priority(mbuf,psid_priority_array->u.permissions_list.len,
...
*/
			bitnum=head_bit_num(mbuf);
			data_length = variablelength_data_num(mbuf,bitnum);
			psid_array->u.permissions_list.len = data_length/4;
			...
			psid_array->u.permissions_list.buf=(psid*)malloc(data_length);
			if(NULL == psid_array->u.permissions_list.buf)
				return -1;

			for(decode_len_sum=0,i=0;decode_len_sum < data_length;i++){
				*(psid_array->u.permissions_list.buf + i) = get32(mbuf);
				be_to_host32(*(psid_array->u.permissions_list.buf + i));
...
static u32 buf_2_psid_ssp(  u8* buf,u32 len,psid_ssp* psid_ssp){
	...
	psid_ssp->psid= get32(mbuf);
	be_to_host32(psid_ssp->psid);
	mbuf+=4;
	size-=4;
...
/*	    bitnum=head_bit_num(mbuf);
	   ...
	   if( size<psid_ssp_array->u.permissions_list.len*sizeof(u8)+bitnum)
		   return -1;
	   mbuf+=bitnum;
	   size+=bitnum;
...
*/
			...
			psid_ssp_array->u.permissions_list.buf=(psid_ssp*)malloc(data_length);
			if(NULL == psid_ssp_array->u.permissions_list.buf)
				return -1;

			for(decode_len_sum=0,i=0;decode_len_sum < data_length;i++){
				decode_len = buf_2_psid_ssp(mbuf,psid_ssp_array->u.permissions_list.len,psid_ssp_array->u.permissions_list.buf + i);
			mbuf += decode_len;
			decode_len_sum += decode_len;
			}
...
static u32 buf_2_psid_priority_ssp(u8* buf, u32 len,psid_priority_ssp* psid_priority_ssp){
    ...
    if(size<6)
   	...
   	psid_priority_ssp->psid = get32(mbuf);
	be_to_host32(psid_priority_ssp->psid);
	mbuf+=4;
	size-=4;
...
*/
			...
			psid_priority_ssp_array->u.permissions_list.buf=(psid_priority_ssp*)malloc(data_length);
			if(NULL == psid_priority_ssp_array->u.permissions_list.buf)
				return -1;

			for(decode_len_sum=0,i=0;decode_len_sum < data_length;i++){
				decode_len = buf_2_psid_priority_ssp(mbuf,psid_priority_ssp_array->u.permissions_list.len,
...
static u32 buf_2_root_ca_scope(  u8* buf,  u32 len,root_ca_scope* root_ca_scope){
	...
	if( (root_ca_scope->permitted_holder_types & 1<<4)!=0 ||
		...
		psid_priority_array_length=buf_2_psid_priority_array(mbuf,len,
...
static u32 buf_2_certificate(  u8* buf,  u32 len,certificate* certificate){
	...
	tobesigned_certificate_length=buf_2_tobesigned_certificate(mbuf,len,
...
*/
			...
			signer_identifier->u.certificates.buf=(certificate*)malloc(data_length);
			if(NULL == signer_identifier->u.certificates.buf)
				return -1;

			for(decode_len_sum=0,i=0;decode_len_sum < data_length;i++){
				decode_len = buf_2_certificate(mbuf,signer_identifier->u.certificates.len,
...
*/
			...
			tobesigned_crl->u.entries.buf=(certid10*)malloc(data_length);
			if(NULL == tobesigned_crl->u.entries.buf)
				return -1;

			for(decode_len_sum=0,i=0;decode_len_sum < data_length;i++){
				decode_len = buf_2_certid10(mbuf,tobesigned_crl->u.entries.len,
...
*/
			...
			tobesigned_crl->u.expiring_entries.buf=(id_and_date*)malloc(data_length);
			if(NULL == tobesigned_crl->u.expiring_entries.buf)
				return -1;

			for(decode_len_sum=0,i=0;decode_len_sum < data_length;i++){
				decode_len = buf_2_id_and_date(mbuf,tobesigned_crl->u.expiring_entries.len,
...
static u32 buf_2_tobe_encrypted_certificate_response(  u8* buf, u32 len, tobe_encrypted_certificate_response* tobe_encrypted_certificate_response){
	...
	tobe_encrypted_certificate_response->certificate_chain.buf=(certificate*)malloc(data_length);
	if(NULL == tobe_encrypted_certificate_response->certificate_chain.buf)
		return -1;

	for(decode_len_sum=0,i=0;decode_len_sum < data_length;i++){
	   decode_len = buf_2_certificate(mbuf,tobe_encrypted_certificate_response->certificate_chain.len,
...
*/
	...
	tobe_encrypted_certificate_response->crl_path.buf=(crl*)malloc(data_length);
	if(NULL == tobe_encrypted_certificate_response->crl_path.buf)
		return -1;

	for(decode_len_sum=0,i=0;decode_len_sum < data_length;i++){
	   decode_len = buf_2_crl(mbuf,tobe_encrypted_certificate_response->crl_path.len,
...
static u32 buf_2_tobesigned_data(  u8* buf,   u32 len, tobesigned_data* tobesigned_data,content_type type){
	...
	switch(type){
		...
		case SIGNED_PARTIAL_PAYLOAD:
			tobesigned_data->u.type_signed.psid= get32(mbuf);
			be_to_host32(tobesigned_data->u.type_signed.psid);
			mbuf+=4;
			size-=4;
		...
		case SIGNED_EXTERNAL_PAYLOAD:
			tobesigned_data->u.psid= get32(mbuf);
			be_to_host32(tobesigned_data->u.psid);
			mbuf+=4;
			size-=4;
			break;
...
*/
		...
		tobesigned_data->flags_content.extensions.buf=(tbsdata_extension*)malloc(data_length);
		if(NULL == tobesigned_data->flags_content.extensions.buf)
			return -1;

		for(decode_len_sum=0,i=0;decode_len_sum < data_length;i++){
			decode_len = buf_2_tbsdata_extension(mbuf,tobesigned_data->flags_content.extensions.len,
...
static u32 buf_2_tobe_encrypted(  u8* buf,   u32 len, tobe_encrypted* tobe_encrypted) {
	...
	switch(tobe_encrypted->type){
		...
		case CERTIFICATE_RESPONSE:
			response_length=buf_2_tobe_encrypted_certificate_response(mbuf,len,&tobe_encrypted->u.response);
		...
		case CERTIFACATE_RESPONSE_ACKNOWLEDGMENT:
			tobe_encrypted_length=buf_2_tobe_encrypted_certificate_response_acknowledgment(mbuf,len,
...
static u32 buf_2_ecies_nist_p256_encrypted_key(  u8* buf,   u32 len, ecies_nist_p256_encrypted_key* ecies_nist_p256_encrypted_key) {
    ...
    elliptic_length=buf_2_elliptic_curve_point(mbuf,len,&ecies_nist_p256_encrypted_key->v,ECIES_NISTP256);
    if(0==elliptic_length)
		return -1;
	mbuf+=elliptic_length;
	size-=elliptic_length;
...
*/
	...
	encrypted_data->recipients.buf=(recipient_info*)malloc(data_length);
	if(NULL == encrypted_data->recipients.buf)
		return -1;

	for(decode_len_sum=0,i=0;decode_len_sum < data_length;i++){
		decode_len = buf_2_recipient_info(mbuf,encrypted_data->recipients.len,
...
*/
		...
		tobesigned_wsa->flags_content.extension.buf=(tbsdata_extension*)malloc(data_length);
		if(NULL == tobesigned_wsa->flags_content.extension.buf)
			return -1;

		for(decode_len_sum=0,i=0;decode_len_sum < data_length;i++){
			decode_len = buf_2_tbsdata_extension(mbuf,tobesigned_wsa->flags_content.extension.len,
