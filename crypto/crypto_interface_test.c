...
static void printf_mstring(char* name,char* buf,int len){
    printf("%s %d\n",name,len);
    int i;
    for(i=0;i<len;i++){
...
FILE *fd;
fd = fopen("../cert/x","r");
fread(publickey_x,1,32,fd);
fd = fopen("../cert/y","r");
fread(publickey_y,1,32,fd);
fd = fopen("../cert/p","r");
fread(privatekey,1,32,fd);

printf_mstring("x",publickey_x,publickey_x_len);
printf_mstring("y",publickey_y,publickey_y_len);
printf_mstring("p",privatekey,private_klen);
/*
...
//int yyy = ECIES_encrypto_message(mess, mess_len, publickey_x, publickey_x_len, publickey_y, publickey_y_len, ephe_public_key_x, &ephe_public_key_x_len, ephe_public_key_y, &ephe_public_key_y_len, en_mess, &en_mess_len, tag, &tag_len);

//printf("ECIES加密数据长度 = %d\n",en_mess_len);
//printf("ECIES加密数据为：%s\n", en_mess);

fd = fopen("../cert/xx","r");
fread(ephe_public_key_x,1,32,fd);
fd = fopen("../cert/yy","r");
fread(ephe_public_key_y,1,32,fd);
fd = fopen("../cert/tag","r");
fread(tag,1,20,fd);
fd = fopen("../cert/encrypted_mess","r");
fread(en_mess,1,10,fd);

printf_mstring("xx",ephe_public_key_x,ephe_public_key_x_len);
printf_mstring("yy",ephe_public_key_y,ephe_public_key_y_len);
printf_mstring("encrypted_mess",en_mess,en_mess_len);
printf_mstring("tag",tag,tag_len);
...
int xxx = ECIES_decrypto_message(en_mess, 10, ephe_public_key_x, ephe_public_key_x_len, ephe_public_key_y, ephe_public_key_y_len, tag, tag_len, privatekey, private_klen, de_mess, &de_mess_len);

printf_mstring("mess",de_mess,de_mess_len);
printf("xxx :%d   %s\n",xxx,de_mess);
