 ...
 *	@len	 key_buf 的字节长度
 *	返回值： 0 私钥获取失败，-1 申请内存不够  other 获取成功
 */
 ...
 */
int		ECDSA_sign_message(char* private_key_buf, int prlen, char* mess_buf, int mess_len, char* signed_mess_buf, int signed_mess_len);
/**
 *	利用公钥进行认证
 ...
 *	@signed_mess_buf 	签名消息长度
 *	@message_buf 没有签名的消息
 *	@mess_len	 未签名消息的长度
 *	返回值 0 表示认证失败，other 签名成功
 */
int		ECDSA_verify_message(char* public_key_x_buf, char* public_key_y_buf, int pulen, char* signed_mess_buf, int signed_mess_len,  char mess_buf, int mess_len);	
/**
 *	获取ECIES私钥
 ...
 */
int		ECIES_get_public_key(char* public_key_x_buf, char* public_key_y_buf, int pulen, char* private_key_buf, int prlen);
/** 公钥加密，私钥解密，利用公钥加密
 *	message_buf 待加密的数据
 *	mess_len	待加密的数据长度
 ...
 *	pulen 	私钥
 */
int		ECIES_encrypto_message(char* mess_buf, int mess_len, char* encrypto_mess_buf, int encrypto_mess_len,char* public_key_x_buf, char* public_key_y_buf, int pulen);
/**	利用私钥解密
 ...
 *  @prlen 
 */
int 	ECIES_decrypto_message(char* encrypto_mess_buf, int encrypto_mess_len, char* decrypto_mess_buf, int decrypto_mess_len, char* private_key_buf, int prlen);
/**
 *	计算消息的哈希值
 *	@message 待机算的消息
 *	@len 	 消息长度
 *	@hash_message 消息的哈希值
 *	@hash_message 已申请内存字节长度
 */
